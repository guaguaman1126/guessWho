...
- 只有房主且房間為 `lobby` 時，可以選擇卡片數量；大廳內兩位玩家都可以上傳或替換圖片、修改名稱；`playing` 與 `paused` 都不允許變更。
...
- 完成口頭問答後，當前玩家按下「已完成提問」，Server 立即把回合切換給另一位玩家。
- 不提供額外的「結束回合」事件或按鈕；切換回合與完成提問是同一筆 Server 操作。
...
| 狀態 | 允許操作 | 禁止操作 |
|---|---|---|
| `lobby` | 加入／離開、房主改卡片數量、雙方改卡片內容、依準備規則選目標、非房主準備／取消準備、房主開始 | 房主準備、蓋牌、提問、猜測 |
| `playing` | 當前玩家完成提問或猜測；每位玩家調整自己的蓋牌 | 修改卡組、改目標、改準備、非當前玩家操作回合 |
...
- 建立房間只能選 `9`、`16` 或 `25` 張；只有房主能修改卡片數量，但大廳內雙方都能修改卡片名稱與圖片。
