...
const scenarios: { value: Scenario; label: string }[] = [
  ...
  { value: "opponent", label: "遊戲中／對手回合" }, { value: "question", label: "已完成口頭提問" },
...
function RoomView({ roomId, count, controller: demo, mode }: Props & { controller: Controller; mode: "demo" | "live" }) {
  ...
  const canGuess = state.status === "playing" && myTurn && state.turnAction === null;
  ...
  const statusText = lobby ? "先選一張，藏好你的秘密。" : paused ? "留一個位置，等朋友回來。" : myTurn ? (state.turnAction === "question" ? "線索到手，換對方想一想。" : "輪到你了，問個好問題。") : "仔細觀察，下一個就是你。";
  ...
  return <main className="room-shell flex flex-col overflow-hidden" data-active={!lobby}>
    ...
    <footer className="shrink-0 px-3 pt-3 pb-2 sm:px-7">
      <div className="mx-auto max-w-3xl">
        <div className="mb-2 min-h-4 text-center text-xs" aria-live="polite">{error ? <span role="alert" className="text-danger">{error}</span> : notice || (lobby ? startReason || "條件都滿足了，可以開始！" : paused ? "暫停期間不能進行遊戲操作" : myTurn ? state.turnAction ? "已記錄提問，請結束回合" : "口頭提問一次，或點卡片指認" : "等待對手行動；你仍可整理自己的蓋牌")}</div>
        <div className="flex items-center justify-center gap-2 sm:gap-4">
          ...
          {lobby ? isHost ? <Button disabled={busy || loading || !!startReason} onClick={() => demo.send({ type: "game:start", payload: {} })} className="min-w-32 sm:min-w-48">開始遊戲 →</Button> : <Button disabled={busy || loading || (!me.ready && !target)} onClick={() => demo.send({ type: "player:ready", payload: { ready: !me.ready } })} className="min-w-32">{me.ready ? "取消準備" : "我準備好了"}</Button>
            : <Button disabled={busy || loading || paused || !myTurn} onClick={() => demo.send({ type: state.turnAction === "question" ? "turn:end" : "turn:question-complete", payload: {} })} className="min-w-32 sm:min-w-48">{paused ? "等待重連" : !myTurn ? "對手的回合" : state.turnAction === "question" ? "結束回合 →" : "已完成提問 ✓"}</Button>}
    ...
    {panel === "rules" && <Modal title="一問一答，找到那個人" onClose={() => setPanel(null)}><p className="text-sm leading-8">每回合選擇口頭提問或指認。提問完按「已完成提問」，再結束回合；猜錯自動換回合，猜中獲勝。蓋牌是你自己的排除筆記，不消耗回合，也能隨時翻回。</p></Modal>}
