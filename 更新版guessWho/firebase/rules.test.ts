...
test("Storage 允許大廳內兩位玩家上傳合格 JPEG", async () => {
  ...
  await assertSucceeds(uploadBytes(ref(environment.authenticatedContext("guest").storage(), "rooms/ABC123/cards/1/guest.jpg"), bytes, { contentType: "image/jpeg" }));
  ...
  await environment.withSecurityRulesDisabled(async context => {
    await setDoc(doc(context.firestore(), "rooms/ABC123"), { hostUid: "host", playerUids: ["host", "guest"], status: "playing" });
  });
  await assertFails(uploadBytes(ref(environment.authenticatedContext("host").storage(), "rooms/ABC123/cards/1/host-playing.jpg"), bytes, { contentType: "image/jpeg" }));
  await assertFails(uploadBytes(ref(environment.authenticatedContext("guest").storage(), "rooms/ABC123/cards/1/guest-playing.jpg"), bytes, { contentType: "image/jpeg" }));
  assert.ok(true);
});
